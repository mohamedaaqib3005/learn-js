...
const userInput = () => {
  ...
  while (input !== "stop") {
    input = prompt("Enter your age or stop")
    console.log(input)
  }
...
} while (i < 5); // Condition is false, but block already ranCopied!
...
// |------|-------------|--------|
// | `for` | You know the number of iterations (e.g., arrays, counters) | `for (let i = 0; i < 5; i++)` |
// | `while` | You don't know how many times — loop until a condition is met | `while (input !== 'quit')` |
// | `do...while` | You want the loop to run **at least once** | Asking for a password or number |

// ---

// ### Key Differences
// - **`for`**: Best for counting and arrays.
// - **`while`**: Checks condition **before** running — may not run at all.
